...
def RQR_loss(model, y, x, q_list, device, args, weights=None):
    num_pts = y.size(0)

    tau = 1-q_list[0]*2
    num_tau = 1
    
    l_rep = torch.tensor([tau]).to(device=device).view(-1, 1).repeat(1, num_pts).view(-1, 1)   
    num_l = l_rep.size(0)
    
    
    
    y_stacked = y.repeat(num_tau, 1)

    x_stacked = x.repeat(num_tau, 1)
    x_stacked = torch.cat([x_stacked, l_rep], dim=1)
    
    pred_y = model(x_stacked)

    y_pred_q1 = pred_y[:,0]
    y_pred_q2 = pred_y[:,1]
   
    y_pred_q1,y_pred_q2 = torch.minimum(y_pred_q1,y_pred_q2),torch.maximum(y_pred_q1,y_pred_q2)


    errors1 = y_stacked -y_pred_q1 
    errors2 = y_stacked- y_pred_q2
    errors = y_pred_q2-y_pred_q1 
        
    loss1 = torch.maximum(errors1*errors2*(tau), errors2*errors1*(tau-1))
    ...
    # print("SHapes")
    # print(y_pred_q1.shape)
    # print(y_pred_q2.shape)
    # print(y[:,0].shape)
    
    independence_loss = independence_penalty(y, y_pred_q1.view(1,-1), y_pred_q2.view(1,-1), pearsons_corr_multiplier, hsic_multiplier)


    loss = pi_loss + independence_loss
