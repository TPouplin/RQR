...
import random
from utils.datasets import scale_data, GetDataset
import pandas as pd

from utils.q_model_ens import MSEModel

results_path = 'RQR/results/'
dataset_base_path = "RQR/datasets/"
REAL_DATA = 'real data'
...
def save_results(dataset_name, data_type, x_train, unscaled_x_train, x_test, unscaled_x_test, y_train, y_test,
    ...
    print("Seed : ", seed)
    print("Mean interval length :", return_values['interval_len'])
    print("Mean coverage :", return_values['coverage'])
    print("HSIC :", return_values['test_hsic'])
    print("Pearson correlation : ", return_values['test_pearson_corr'])
    pd.DataFrame(return_values, index=[seed]).to_csv(results_path)


dict_loss = {"batch_qr":"OQR","RQR":"RQR-O"}
...
def args_to_txt(args):
    if args.method == 'wqr':
        ...
        args_summary = str("loss=" + dict_loss[args.loss] + "_bs=" + str(1024) + "_corr_mult=" +
                           str(args.corr_mult) + '_hsic_mult=' + str(args.hsic_mult))
    return args_summary


def get_method_results_dir(dataset_type, data_name, args):
    return f"{results_path}{"RQR-O"}/{data_name}/{args_to_txt(args)}"
...
def get_feature_as_function_of_mult_figure_dir(dataset_type, data_name, mult, args):
    ...
    return f"{results_path}{"RQR-O"}/{data_name}/{args_txt}"
...
def calculate_results(x_train, y_train, x_test, y_test, test_y_upper, test_y_lower, train_y_upper, train_y_lower, y_range,
